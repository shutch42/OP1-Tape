...
class Track:
    ...
    def __init__(self):
        self.wr = wvm.open("audio/side_b.wav", "rb")
        self.ww = wvm.open("audio/tmp.wav", "wb")

        self.p = pyaudio.PyAudio()

        self.fast_stream = self.p.open(format=self.p.get_format_from_width(self.wr.getsampwidth()),
                                       channels=self.wr.getnchannels(), rate=self.wr.getframerate()*5, output=True,
                                       frames_per_buffer=self.CHUNK_SIZE)

        # self.slow_stream = self.p.open(format=self.p.get_format_from_width(self.wf.getsampwidth()),
        #                                channels=self.wf.getnchannels(), rate=int(self.AUDIO_RATE/2), output=True,
        #                                frames_per_buffer=self.CHUNK_SIZE)

        self.normal_stream = self.p.open(format=self.p.get_format_from_width(self.wr.getsampwidth()),
                                         channels=self.wr.getnchannels(), rate=self.wr.getframerate(),
    ...
    def play_chunk(self):
        self.stream = self.normal_stream
        self.data = self.wr.readframes(self.CHUNK_SIZE)
    ...
    def reverse_chunk(self):
        ...
        position = self.wr.tell()

        # Move file position back two chunks
        self.wr.setpos(position - self.CHUNK_SIZE * 2)
    ...
    def get_time(self):
        return self.wr.tell() / self.AUDIO_RATE
